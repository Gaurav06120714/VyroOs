...
$(BUILD)/xhci.o: kernel/xhci.c
	$(CC) $(CFLAGS) kernel/xhci.c -o $(BUILD)/xhci.o
...
$(BUILD)/vyro.img: $(BUILD)/boot.bin $(BUILD)/kernel.bin
	...
	@echo "  [IMG]   vyro.img ready (boot=$(shell wc -c < $(BUILD)/boot.bin)b kernel=$(shell wc -c < $(BUILD)/kernel.bin)b / 393216b max)"
	@if [ $$(wc -c < $(BUILD)/kernel.bin) -gt 393216 ]; then echo "  [WARN]  kernel exceeds 384KB! Increase sector count in boot.asm"; fi
