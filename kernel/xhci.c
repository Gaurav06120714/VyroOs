...
int xhci_event_ring_setup(void) {
    ...
    *(volatile uint16_t*)(erst + 8) = 256;
    ...
    mmio_w32(runtime + 0x20 + XHCI_RT_IR0_ERSTSZ, 1);
...
int xhci_event_poll(uint8_t* trb_type, uint8_t* completion_code, uint8_t* slot_id) {
    ...
    if (cycle != event_cycle_bit) return 0;
    ...
    mmio_w32(runtime + 0x20 + 0x38, (uint32_t)(erdp & 0xFFFFFFF0) | (1u << 3));
