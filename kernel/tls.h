...
// Build a TLS 1.3 Certificate handshake message (RFC 8446 §4.4.2).
//   - request_context = 0 bytes
//   - one CertificateEntry with the given DER blob and zero extensions
// Writes a complete TLS handshake message (4-byte handshake header + body) into `out`.
...
// Compute the server-side Finished handshake message body (32 bytes) given the
// server handshake traffic secret and the transcript hash up to and including
// CertificateVerify. Writes 4-byte handshake header + 32-byte MAC into `out` (36 bytes).
int tls_build_server_finished(uint8_t* out, uint32_t out_max,
                              const uint8_t server_hs_traffic_secret[32],
                              const uint8_t transcript_hash[32]);

// Build a TLS 1.3 ServerHello record. Echoes the client's session_id.
// `server_random` is 32 bytes generated by the server (caller). Returns total
// length on success, -1 on overflow.
