...
// prog-if values → controller type
...
typedef struct {
    ...
    uint16_t bcdUSB;              // USB version (BCD)
...
// USB setup packet (8 bytes) — sent on control transfers
typedef struct {
...
int         usb_init();                  // returns 1 if controller found
