...
VYRO_COMPOSITOR_DEPENDENCIES = libdrm libinput libvyro-linux

define VYRO_COMPOSITOR_BUILD_CMDS
    $(TARGET_CC) $(TARGET_CFLAGS) -O2 -Wall \
        $(@D)/src/main.c $(@D)/src/server.c $(@D)/src/chrome.c $(@D)/src/input.c \
        $(shell $(PKG_CONFIG_HOST_BINARY) --cflags libdrm libinput) \
        $(shell $(PKG_CONFIG_HOST_BINARY) --libs   libdrm libinput) \
        -o $(@D)/vyro-compositor
endef
